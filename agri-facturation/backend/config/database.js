...
async function initialize() {
    try {
        ...
        console.log('Pool de connexion Oracle cree avec succes');
        return true;
    } catch (err) {
...
async function getConnection() {
    ...
    if (!pool) {
        throw new Error('Le pool de connexion n\'est pas initialise');
    }
...
async function testConnection() {
    try {
        ...
        console.log('Connexion Oracle testee:', result.rows[0]);
        return true;
    } catch (err) {
        console.error('Test connexion echoue:', err);
        return false;
    }
}
